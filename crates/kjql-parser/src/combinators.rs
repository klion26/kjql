...
pub(crate) fn parse_number(input: &str) -> IResult<&str, Index> {
    map_res(recognize(digit1), |s: &str| s.parse::<usize>().map(Index))(input)
}
...
{
    alt((
        value(LensValue::Bool(true), tag("true")),
        value(LensValue::Bool(false), tag("false")),
    ))
}
...
mod tests {
    ...
    fn check_parse_object_index() {
        ...
        assert_eq!(
            parse_object_index()("{1,2,3}").unwrap(),
            ("", vec![Index(1), Index(2), Index(3)])
        );
        assert!(parse_object_index()("{}").is_err());
        assert!(parse_object_index()("{1,2,3").is_err());
    }
    ...
    fn check_parse_object_range() {
        ...
        assert!(parse_object_range()("{1:3").is_err());
    }
