...
mod tests {
    ...
    use crate::{core::walker, flatten_json_array::flatten_json_array, truncate::truncate_json};
    ...
    const FLATEEN_DATA: &str = r#"[[[[[[[[[[[[[[1]]]]]]]]]]]]], [[[[[2]]]], 3], null]"#;
    ...
    fn get_range() {
        let json: Value = serde_json::from_str(DATA).unwrap();
        let range_selector = r#""range".[2:5]"#;
    ...
    fn get_out_of_bound_range() {
        ...
        assert_eq!(
            Err(String::from(
                "Range [6:7] is out of bound, node \"range\" has a length of 7"
            )),
            walker(&json, selector)
        );
    }
    ...
    #[test]
    fn get_property_as_range_with_not_start() {
        let json: Value = serde_json::from_str(DATA).unwrap();
    ...
    #[test]
    fn get_property_as_out_of_bound_range_is_filter() {
        let json: Value = serde_json::from_str(DATA).unwrap();
        let selector = r#""nested-filter"|{[1:3]}"#;
        assert_eq!(
            Err(String::from(
    ...
    fn check_lenses() {
        ...
        assert_eq!(
            Ok(
                json!([{"alpha": 1, "beta": Value::Null}, {"gamma": 3, "delta": "something"}, {"alpha": 7}, {"delta": 4}])
            ),
        ...
        assert_eq!(
            Ok(json!([{"alpha": 1, "beta": Value::Null},])),
            walker(&json, selector_key_value_null)
        );

        assert_eq!(
            Ok(json!([{"gamma": 3, "delta": "something"}])),
